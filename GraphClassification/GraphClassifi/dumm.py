...
class GCN(nn.Module):
    def __init__(self, in_feats, h_feats, num_classes):
        ...
        self.conv2 = GraphConvolution(h_feats, h_feats)
        self.conv3 = GraphConvolution(h_feats, num_classes)

    def forward(self, g,in_feat ):#in_feat : 100,10, g = 1, 100,100
        h = self.conv1(in_feat, g)  #g : tensor(1, 100, 100), in_feat : Tensor(100,10)
        h = F.relu(h) # h = 100, 15
        h = self.conv2(h,torch.empty(100,100)) # 15x 100    100x 100   -> 15x100
        #h = F.relu(h)
        h = self.conv3(h,torch.ones(10,100))
