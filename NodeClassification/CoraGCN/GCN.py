...
paper_features_label = np.genfromtxt(path / 'cora.content',
                                     dtype=np.str)  # 데이터셋, (2078, 1435), [[논문 id, fea..ture...., 'label'] ,...,[논문 id, fea..ture...., 'label']]
...
lbl2idx = {k: v for v, k in enumerate(sorted(np.unique(
    labels)))}  # 라벨에 인덱싱(라벨-> 숫자) {'Case_Based': 0, 'Genetic_Algorithms': 1, 'Neural_Networks': 2, 'Probabilistic_Methods': 3, 'Reinforcement_Learning': 4, 'Rule_Learning': 5, 'Theory': 6}
...
adj = normalize(
    adj + eye(adj.shape[0]))  # Normalizing for removing gradient vanishing and exploding problem 기울기 소실과 폭주가 어떤 식으로 발생?
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout):  # nfeat : 1433     nhid : 20     nclass : 7
        ...
        self.gc1 = GraphConvolution(nfeat,
                                    nhid)  # 입력 데이터의 차원과 hidden layer의 차원                                  # self : gc1) : GraphConvolution(1433->20)
        self.gc2 = GraphConvolution(nhid,
                                    nclass)  # hidden layer의 차원, 출력 데이터의 차원(해당 예제에서는 논문의 label 수)   # self : : GraphConvolution(20->7), in_features : 20, out_features : 7, bias = True
